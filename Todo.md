...
#### 🚨 긴급 이슈 해결 (기능점검결과 반영)
- [ ] **JWT 기반 사용자 로그인 세션 관리**: FastAPI-Users를 활용한 안전한 인증 시스템 구현 (Redis 세션 저장)
- [ ] **3턴 멀티턴 대화 시스템**: 사용자별 대화 세션 관리 및 자동 요약 기능 구현
- [ ] **대화 히스토리 관리**: 채팅 메시지 저장, 요약된 대화 저장, 컨텍스트 유지 기능
- [ ] **권한 기반 접근 제어**: 역할별 권한 관리 (admin, user, developer) 및 API 키 기반 인증
- [ ] **LangFlow 워크플로우 관리**: 저장, 조회, 삭제 API 및 개발/배포 상태 관리
- [ ] **OpenAI 호환 임베딩 API 구현**: `/v1/embeddings` 엔드포인트 구현 (현재 404 Not Found)
- [ ] **벡터 문서 추가 기능 개선**: 임베딩 서비스 연동 로직 점검 및 success_count 이슈 해결
- [X] **API 문서 개선**: curl 명령어 사용법 및 JSON 전송 방법 문서화 완료

#### CoE-RagPipeline
- [ ] **레포지토리간 연관관계 분석 고도화**: 공통 의존성, 코드 패턴, API 호출 관계, 개발자 협업 네트워크 분석 강화
- [ ] **LLM 기반 가이드 생성 API**: `/api/v1/generate/guide` 엔드포인트로 개발가이드, 공통코드, 재활용 함수 가이드 자동 생성
- [ ] **지능형 청킹 시스템**: 함수/클래스/섹션 단위 의미 기반 문서 분할 및 메타데이터 관리 강화
...
#### CoE-Backend
- [ ] **AI4X 모델 기반 도구 선택 최적화**: 의미적 유사성 기반 도구 선택 및 코사인 유사도 최적화 구현
- [ ] **도구 임베딩 캐싱 시스템**: 도구 설명의 임베딩을 사전 계산하여 캐싱하는 성능 최적화
...
- [ ] **코드 어시스턴트 API 확장**: 기존 코딩 어시스턴트 도구들을 통합하여 `/api/coding-assistant/*` API 엔드포인트 완성
