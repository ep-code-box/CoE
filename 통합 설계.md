...
- **API**: `/analyze`, `/results`, `/search`, `/embeddings/stats`, `/realtime/analyze`, `/patterns/extract`

### 3.2. CoE-Backend (AI 에이전트/API 계층)
- **역할**: AI 추론 및 외부 인터페이스 제공
- **주요 기능**:
  - LangGraph 기반의 동적 에이전트 실행
  - 사용자의 요청에 따라 `guide_extraction_tool`, `code_assistant_tool` 등 적절한 도구 선택 및 실행
  - `CoE-RagPipeline`의 검색 API를 호출하여 RAG 컨텍스트 확보
  - 확보된 컨텍스트를 바탕으로 LLM에 프롬프트 전달 및 결과 생성
  - OpenWebUI, LangFlow 등과 호환되는 OpenAI 표준 API 제공
  - **코드 어시스턴트 기능**: 코드 생성, 리팩토링, 버그 수정, 코드 리뷰 등 개발 지원
- **API**: `/v1/chat/completions`, `/v1/models`, `/flows/*`, `/code/*`
